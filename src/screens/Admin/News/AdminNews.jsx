...
function AdminNews({ navigation }) {
  ...
  const getNews = async () => {
    await axios
      .get(
        "https://life-on-land-backend.azurewebsites.net/api/news/adminGetAllNews"
      )
      .then((res) => {
  ...
  const approveNews = async (newsId) => {
    Alert.alert("Approve News", "Are you sure you want to approve this news?", [
      {
        text: "Ok",
        onPress: async () => {
          await axios
            .patch(
              `https://life-on-land-backend.azurewebsites.net/api/news/adminApproveNews/${newsId}`
            )
            .then((res) => {
              if (res.data.status) {
                Alert.alert("Success", "News approved successfully", [
                  {
                    text: "OK",
                    onPress: () => getNews(),
                  },
  ...
  const RejectNews = async (newsId) => {
    Alert.alert("Reject News", "Are you sure you want to reject this News?", [
      {
        text: "Ok",
        onPress: async () => {
          await axios
            .patch(
              `https://life-on-land-backend.azurewebsites.net/api/news/adminRejectNews/${newsId}`
            )
            .then((res) => {
              if (res.data.status) {
                Alert.alert("Success", "News reject successfully", [
                  {
                    text: "OK",
                    onPress: () => getNews(),
                  },
  ...
  const filterData = (news, searchKey) => {
    const result = news.filter((newss) =>
      newss.newsTittle.toLowerCase().includes(searchKey)
    );
    setNews(result);
  };
    ...
    Alert.alert("Delete News", "Are you sure you want to delete this News?", [
      {
        ...
        onPress: async () => {
          await axios
            .delete(
              `https://life-on-land-backend.azurewebsites.net/api/news/deleteNews/${newsId}`
            )
            .then((res) => {
              if (res.data.status) {
                ...
                {news.map((item) => (
                  <View>
                    {item.adminStatus == "Pending" ? (
                      <TouchableOpacity
                        onPress={() =>
                          navigation.navigate("AdminSpecificNews", {
                            newsId: item._id,
                          })
                        }
                      >
