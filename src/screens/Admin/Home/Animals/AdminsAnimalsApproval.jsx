...
function AdminsAnimalsApproval(props) {
  ...
  const updateAnimalStatus = () => {
    axios
      .patch(
        `https://life-on-land-backend.azurewebsites.net/api/forest/adminApprove/${animalId.animalId}`
      )
      .then((res) => {
  ...
  const rejectAnimalStatus = () => {
    axios
      .patch(
        `https://life-on-land-backend.azurewebsites.net/api/forest/adminReject/${animalId.animalId}`
      )
      .then((res) => {
        console.log(res.data);
    ...
    <View style={styles.container}>
      <View style={styles.backgroundStack}>
        <View style={styles.frame61}>
          <Text style={styles.sriLankanLeopard}>{animalId.animalName}</Text>
        </View>
        <Image
          source={{ uri: animalId.animalImage }}
          resizeMode="contain"
          style={styles.image1}
        ></Image>
        <View style={styles.scrollArea1}>
          ...
          >
            <View style={styles.rect1}>
              <Text style={styles.loremIpsum1}>{animalId.animalDetails}</Text>
            </View>
          </ScrollView>
        </View>
        <TouchableOpacity
